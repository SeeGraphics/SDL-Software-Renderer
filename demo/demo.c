...
int main(void) {
  ...
  while (demo.running) {
    ...
    while (SDL_PollEvent(&demo.game.event)) {
      demo_handle_event(&demo, &demo.game.event);
    }

    demo_frame(&demo, now, dt);
  }
